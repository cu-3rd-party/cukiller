...
class PlayerService:
    ...
    async def save_player_to_redis(self, player: Player) -> bool:
        ...
        try:
            ...
            await self.redis_client.set(
                key, json.dumps(player_data, default=str)
            )

            # Also index by user and game for faster lookups
            await self.redis_client.sadd(
                f"user_players:{player.user.id}", player.id
            )
            await self.redis_client.sadd(
                f"game_players:{player.game.id}", player.id
            )
    ...
    async def get_player_from_redis(
        self, player_id: int
    ...
    async def get_player_by_user_game(
