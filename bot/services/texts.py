...
_TEXTS: dict[str, str] = {
    ...
    "registration.welcome": (
        "Привет, новобранец! Изучи правила оформления профиля, укажи фамилию и имя и заполни свои данные"
    ),
    ...
    "registration.pending": (
        "Главное, в ходе следственных действий не выйти на самих себя. Твое досье "
        "проходит проверку, оперативно сообщим результат"
    ),
    # Participation
    ...
    "leave.result": (
        "Вы вышли из игры и стали NPC с богатым прошлым. Но бывших не бывает... "
    ...
    "moderation.user_denied_new_reason": (
        "Заявка не прошла внутреннюю проверку, следователь передал сообщение: {reason}"
    ),
    "moderation.user_denied_update": "Ваши изменения не соответствуют нашим правилам. Закон — он один для всех",
    "moderation.user_denied_update_reason": (
        "Попытка засчитана, но правила строже. Ваши изменения отклонены. Причина: {reason}."
    ),
    ...
    "profile.name_line": "\nФамилия: <b>{family_name}</b>\nИмя: <b>{given_name}</b>\n",
    "profile.family_name_required": (
        "Профиль временно заблокирован. Укажи фамилию и отправь досье на проверку — "
        "после подтверждения доступ восстановится"
    ),
...
Администрация имеет право трактовать правила и принимать решения в спорных или
неописанных случаях, чтобы сохранить <b>безопасность</b> и <b>честность</b> игры.

<b>0.1. Умышленная порча игрового процесса</b>
<b>Запрещены</b> действия, совершаемые с целью <i>испортить игру другим участникам</i>,
даже если нарушитель не получает прямой выгоды:
• срыв раскрытий;
• провокации;
...
<b>1.2.</b> <b>Запрещены:</b> физическое или психологическое давление, угрозы, шантаж,
навязчивые действия, «выкручивание» согласия и прочее.

<b>1.3.</b> <b>Запрещено:</b> удерживать человека, хватать вещи, одежду или рюкзак,
провоцировать падение, толкаться.

<b>1.4.</b> <b>Запрещено</b> проникновение в <i>закрытые зоны</i> (служебные помещения,
закрытые комнаты или кабинеты, зоны «только для сотрудников» и т. п.).
...
<b>Всё, что не запрещено, разрешено</b>, при условии что это не нарушает правила и
не противоречит <i>здравому смыслу</i>!
...
<b>3. Раздел «О себе»</b>
<b>Запрещены</b> оскорбления, унижения, дискриминационные высказывания, травля,
угрозы и любой контент, <i>нарушающий правила университета или законодательство</i>.

<i>Администрация вправе</i> потребовать изменить профиль, отклонить заявку на регистрацию,
отказать во внесении изменений в профиль, а в крайнем случае — <b>удалить профиль</b>.
""",
    "rules.game": """
<b>Игровой процесс</b>
...
<b>Шпион</b> и <b>цель</b> должны в боте нажать соответствующие кнопки <b>подтверждения</b>,
либо <b>оспорить</b> раскрытие.
...
<b>Механики</b>
• Подбор цели происходит на основе вашего <b>рейтинга</b> и дополнительных
<i>скрытых характеристик</i>.
• <b>Штраф</b> и <b>награда</b> рассчитываются с учётом количества сыгранных раундов,
рейтинга цели и шпиона; принцип похож на рейтинг <b>Эло</b> (например, в шахматах или го).
""",
}
...
def render(key: str, **kwargs: object) -> str:
    return get(key).format_map(_format_kwargs(kwargs))


def _format_kwargs(values: Mapping[str, object]) -> dict[str, object]:
    return dict(values)
